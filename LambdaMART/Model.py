...
from sklearn.utils import check_X_y, check_consistent_length, check_random_state,validation
from sklearn.utils.validation import NotFittedError

from LambdaMART.metrics import dcg


class LambdaMART(six.with_metaclass(ABCMeta, BaseEnsemble,
                                    _LearntSelectorMixin)):
    def __init__(self, base_estimator, metric=None, learning_rate=0.1, n_estimators=100, query_subsample=1.0,
                 subsample=1.0, min_samples_split=2, min_samples_leaf=1, max_depth=3, random_state=None,
                 max_features=None, verbose=0, max_leaf_nodes=None, warm_start=True, n_jobs=1):
        super().__init__(base_estimator, n_estimators)
        self.n_jobs = n_jobs
        self.metric = dcg.NDCG() if metric is None else metric
